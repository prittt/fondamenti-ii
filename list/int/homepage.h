/**
@mainpage Liste di `int`

In questa pagina trovi la documentazione per liste di `int`. In particolare:

- \ref elemtype.h contiene la definizione di `ElemType` per il tipo `int`
e la documentazione delle funzioni a esso associate;
- \ref list.h contiene la definizione del tipo `Item` e la documentazione delle 
